...
export function useChat(nav: string) {
  ...
  const autosize = () => {
    ...
    if (ta) { ta.style.height = 'auto'; ta.style.height = Math.min(ta.scrollHeight, MAX_COMPOSER_H) + 'px' }
  }
  useEffect(autosize, [text])

  // Hand the caret back so the next question is one keystroke away.
  const focusComposer = () => { setTimeout(() => taRef.current?.focus(), FOCUS_DELAY_MS) }
  ...
  // changed (popstate).
  const clearChat = () => { setThread([]); setChatId(null) }
  ...
  return {
    // composer
    text, setText, pendingImg, focus, taRef, asking,
    ...
    focusComposer,
    // conversation
    thread, chatId, chatIdRef, sendQuestion, stopAsk, showChat, newChat, clearChat,
    // saved list
    chatList, chatTotal, openChat, loadMoreChats, renameChat, deleteChat,
  }
}
