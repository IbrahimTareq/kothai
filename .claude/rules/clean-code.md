...
How code gets written in this repo. Formatting, import boundaries and file
shape are enforced by `pnpm test`; if a rule here becomes mechanically
...
- Every non-obvious guard, ordering constraint or workaround **names the failure
  that caused it**. See the comments above the `/api/health` route in
  `server/router.ts`.
