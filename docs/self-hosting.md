...
| `STASH_AI_EMBED_PROVIDER` | *(unset)* | Only matters when `STASH_AI_PROVIDER=remote` on the full image — the lite image always sends embedding remote regardless. Unset leaves the decision to whether an endpoint embedding model is named in Settings — naming one sends the role out, clearing it keeps it here. That covers both cases without a switch: OpenAI and OpenRouter serve `/embeddings`, while Ollama Cloud, Groq and Anthropic serve none at all. Set this to `local` or `remote` to override that decision in either direction. Either way, moving the embedding role to a different provider re-indexes the whole library once, in the background — vectors from two different embedding models aren't comparable. |
