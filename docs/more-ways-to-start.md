...
The `:once` tag is the full image with the three defaults ONCE requires already set (`PORT=80`, `STASH_HOME=/storage`, a `/up` healthcheck), because ONCE installs an image by name and has nowhere to put environment variables. It's the same build as `:latest` otherwise.

> [!IMPORTANT]
> ONCE handles TLS but adds no authentication of its own, and it gives Kothai a public hostname. Set `STASH_PASSWORD`.
