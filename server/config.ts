...
//
// Resolution order per value: specific env var → derived from KOTHAI_HOME → default.
// Vars are KOTHAI_-prefixed because PaaS and self-hosted environments inject a
...
export function resolveConfig(env: NodeJS.ProcessEnv = process.env, root: string = ROOT): Config {
  const home = env.KOTHAI_HOME ? path.resolve(root, env.KOTHAI_HOME) : null
  ...
  const DATA_DIR = pick('KOTHAI_DATA_DIR', 'data')
  return {
    ...
    ALLOW_PRIVATE_FETCH: ['1', 'true'].includes((env.KOTHAI_ALLOW_PRIVATE_FETCH || '').toLowerCase()),
    ...
    PASSWORD: env.KOTHAI_PASSWORD || null,
  }
}
...
// KOTHAI_AI_BASE_URL is set, the key must come from the environment too. Mixing
...
export function resolveAiConfig(env: NodeJS.ProcessEnv = process.env, creds: AiCredentials | null = null): AiConfig {
  ...
  const source = env.KOTHAI_AI_BASE_URL
    ? { baseUrl: strip(env.KOTHAI_AI_BASE_URL), apiKey: env.KOTHAI_AI_API_KEY || null, providerId: null }
    : creds?.baseUrl
  ...
  // An endpoint from either source implies remote. KOTHAI_AI_PROVIDER=remote
  ...
  const provider = env.KOTHAI_AI_PROVIDER === 'remote' || source.baseUrl ? 'remote' : 'local'
