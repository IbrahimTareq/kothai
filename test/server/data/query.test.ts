...
test('applyFilters narrows by type, source, q, and collection set', () => {
  assert.deepEqual(
    applyFilters(LIB, { type: 'video' }).map(n => n.id),
    ['a', 'b'],
  )
  assert.deepEqual(
    applyFilters(LIB, { source: 'reels' }).map(n => n.id),
    ['a'],
  )
  assert.deepEqual(
    applyFilters(LIB, { q: 'makkah' }).map(n => n.id),
    ['a', 'd'],
  )
  assert.deepEqual(
    applyFilters(LIB, { collection: new Set(['b', 'e']) }).map(n => n.id),
    ['b', 'e'],
  )
...
test('pageOf slices with clamped offset/limit', () => {
  assert.deepEqual(
    pageOf(LIB, 1, 2).map(n => n.id),
    ['b', 'c'],
  )
  assert.deepEqual(
    pageOf(LIB, 4, 10).map(n => n.id),
    ['e'],
  )
...
test('applyFilters: unavailable narrows to marked notes only', () => {
  ...
  assert.deepEqual(
    applyFilters(notes, { unavailable: 'all' }).map(n => n.id),
    ['a', 'b', 'c'],
  )
})
...
test('applyFilters: unavailable notes are out of an ordinary view unless asked for', () => {
  ...
  assert.deepEqual(
    applyFilters(notes, { type: 'video' }).map(n => n.id),
    ['a'],
    'and under any other filter',
  )
  assert.deepEqual(
    applyFilters(notes, { unavailable: 'only' }).map(n => n.id),
    ['b'],
  )
  assert.deepEqual(
    applyFilters(notes, { unavailable: true }).map(n => n.id),
    ['b'],
    'true still means only',
  )
  assert.deepEqual(
    applyFilters(notes, { unavailable: 'all' }).map(n => n.id),
    ['a', 'b'],
...
test('sortNotes: newest first by default', () => {
  assert.deepEqual(
    sortNotes([A, B, C], 'newest').map(n => n.id),
    ['c', 'b', 'a'],
  )
  assert.deepEqual(
    sortNotes([A, B, C], undefined).map(n => n.id),
    ['c', 'b', 'a'],
  )
})
...
test('sortNotes: oldest first is the exact reverse', () => {
  assert.deepEqual(
    sortNotes([C, A, B], 'oldest').map(n => n.id),
    ['a', 'b', 'c'],
  )
})
...
test('sortNotes: an unknown sort falls back to newest rather than throwing', () => {
  assert.deepEqual(
    sortNotes([A, C], 'sideways').map(n => n.id),
    ['c', 'a'],
  )
})
...
test('sortNotes: exact ties keep insertion order, so a just-saved note stays on top', () => {
  ...
  assert.deepEqual(
    sortNotes(notes, 'newest').map(n => n.id),
    ['just-saved', 'older', 'oldest'],
  )
  // And oldest-first reverses ties too, rather than leaving them newest-first.
  assert.deepEqual(
    sortNotes(notes, 'oldest').map(n => n.id),
    ['oldest', 'older', 'just-saved'],
  )
})
