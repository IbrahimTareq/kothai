...
test('without an embeddings path, every role shares the one catalogue', async () => {
  ...
  assert.deepEqual(out.embed.map((o) => o.key), ['chat-a', 'chat-b'])
  assert.deepEqual(out.llm.map((o) => o.key), ['chat-a', 'chat-b'])
})
...
test('with one, the embedding role gets its own list and the others do not', async () => {
  ...
  await p.init()
  const out = await p.listModels()
  assert.deepEqual(out.embed.map((o) => o.key), ['vec-small', 'vec-large'])
  assert.deepEqual(out.llm.map((o) => o.key), ['chat-a', 'chat-b'])
  assert.deepEqual(out.vision.map((o) => o.key), ['chat-a', 'chat-b'])
})
...
test('a failing embeddings probe does not take the provider down with it', async () => {
  routes['/v1/embeddings/models'] = (req, res) => { res.writeHead(500); res.end('nope') }
  const p = createRemoteProvider({ baseUrl: base, apiKey: null, models: {}, embeddingsPath: '/embeddings/models' })
  await p.init()
  const out = await p.listModels()
  assert.deepEqual(out.llm.map((o) => o.key), ['chat-a', 'chat-b'], 'chat still works')
  assert.deepEqual(out.embed.map((o) => o.key), ['chat-a', 'chat-b'], 'and embed falls back')
})

test('the endpoint being down entirely is still one failure, not two', async () => {
  const p = createRemoteProvider({ baseUrl: 'http://127.0.0.1:1/v1', apiKey: null, models: { llm: 'x' }, embeddingsPath: '/embeddings/models' })
  await p.init()
