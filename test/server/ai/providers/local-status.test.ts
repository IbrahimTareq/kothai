...
import { computeAggregate } from '../../../../server/ai/providers/local.ts'
import type { Policy, Role, RoleStatus } from '../../../../server/ai/roles.ts'

// Annotated as RoleStatus so each factory's `state` stays the literal the
// manager reports, rather than widening to string the moment it is returned.
const idle = (model = ''): RoleStatus => ({ state: 'idle', progress: 0, message: '', model })
const off = (): RoleStatus => ({ state: 'off', progress: 0, message: '', model: '' })
const ready = (model: string): RoleStatus => ({ state: 'ready', progress: 100, message: 'Ready', model })
const loading = (model: string, progress: number, message?: string): RoleStatus => ({
  state: 'loading',
  progress,
  message: message ?? `${model}: ${progress}%`,
  model,
})
const error = (model: string, message = 'boom'): RoleStatus => ({ state: 'error', progress: 0, message, model })

type Policies = Record<Role, Policy>

test('all roles off → ready', () => {
  const roles = { llm: off(), embed: off(), vision: off() }
  const policies: Policies = { llm: 'off', embed: 'off', vision: 'off' }
...
test('single always-role loading → loading with its own progress', () => {
  ...
  const policies: Policies = { llm: 'ondemand', embed: 'always', vision: 'ondemand' }
...
test('multi always-role sequential loading averages only over currently-loading roles, not diluted by idle ones', () => {
  ...
  const policies: Policies = { llm: 'always', embed: 'always', vision: 'ondemand' }
...
test('two roles loading simultaneously averages across just those two', () => {
  ...
  const policies: Policies = { llm: 'always', embed: 'always', vision: 'off' }
...
test('an errored ondemand role with nothing else active/loading → ready, not error (scoped to its own next use)', () => {
  ...
  const policies: Policies = { llm: 'off', embed: 'off', vision: 'ondemand' }
  assert.deepEqual(computeAggregate(roles, policies), { state: 'ready', progress: 100, message: 'Ready' })
})
